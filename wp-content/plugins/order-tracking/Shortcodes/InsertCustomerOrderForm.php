...
function Insert_Customer_Order_Form($atts) {
	...
	if ($Email_Field_Text == "") {$Email_Field_Text = __("Order E-mail Address", 'order-tracking');}
	...
	extract( shortcode_atts( array(
		 		'order_status' => '',
		 		'order_location' => '',
		 		'success_message' => __('Thank you. Your order number is: ', 'order-tracking'),
		 		'customer_form_title' => __('Create an Order', 'order-tracking'),
				...
				'customer_instructions' => __('Please fill out the form below to create an order.', 'order-tracking'),
				'submit_text' => __('Send Order', 'order-tracking')),
		$atts
		)
	);
	...
	$ReturnString .= "<p>" . __('The e-mail address to send order updates to, if the site administrator has selected that option.', 'order-tracking')  . "</p>";
	$ReturnString .= "</div>";
	$ReturnString .= "</div>";
