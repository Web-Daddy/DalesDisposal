...
class EWD_OTP_Tracking_Form_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__('Order Tracking Form', 'order-tracking'), // Name
	...
	public function form( $instance ) {
		...
		$order_field_text = ! empty( $instance['order_field_text'] ) ? $instance['order_field_text'] : __( 'Order Field Text', 'EWD_OTP' );
		...
		<p>
		<label for="<?php echo $this->get_field_id( 'order_field_text' ); ?>"><?php _e( 'Order Field Text:', 'EWD_OTP' ); ?></label> 
...
function EWD_OTP_Register_Widgets() {
	register_widget("EWD_OTP_Tracking_Form_Widget");
}
add_action('widgets_init', 'EWD_OTP_Register_Widgets');
